...
async def read_dishes_in_shopcart(account_id, message):
    ...
    for ret in data:
        await bot.send_photo(message.chat.id, ret[3], f'{ret[0]}\nОписание: {ret[1]}\nЦена: {ret[2]}\nКоличество: {ret[4]}',
                             reply_markup=InlineKeyboardMarkup()
                             .add(InlineKeyboardButton(f'Добавить еще', callback_data=f'add_to_shopcart_{ret[5]}'))
                             .add(InlineKeyboardButton(f'Удалить', callback_data=f'delete_from_shopcart_{ret[5]}')))
